...
class Multidrive extends EventEmitter {
  ...
  _initSource (key, opts, cb) {
    ...
    drive.ready(err => {
      ...
      this._findChildren(drive, cb)
    })
  }

  _findChildren (drive, cb) {
    cb = once(cb)
    drive.readdir(P_SOURCES, (err, list) => {
      let pending = 1
      if (err && err.code !== 'ENOENT') return done(err, drive)
      if (!list || !list.length) return done(null, drive)
      pending = list.length
  ...
  addSource (key, cb) {
    ...
    function onmounted (err) {
      // This hack ensures that the parents for the new core are recorded correctly.
      // TODO: Remove once the underlying bug is fixed .
      // See: https://github.com/andrewosh/mountable-hypertrie/pull/2)
      self.corestore.get({ key: hkey, parents: [self.key] })
