...
module.exports = function makeKappa (opts = {}) {
  const { multidrive, viewContext } = opts

  const kappa = new Kappa()

  multidrive.sources(drives => {
    drives.forEach(ondrive)
  })
  multidrive.on('source', ondrive)

  function ondrive (drive) {
    kappa.source(drive.key.toString('hex'), hyperdriveSource, {
      drive,
      mounts: false
    })
  }

  kappa.on('indexed', (name) => {
